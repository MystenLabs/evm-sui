...
This is **showcase 01** of the [Walrus Low-Hanging-Fruit](../README.md) collection. The Solidity contract for this showcase lives in the shared Foundry package at [`../contracts/src/EvmWalNFT.sol`](../contracts/src/EvmWalNFT.sol); everything else is in this folder.
...
# Terminal B — from this folder (showcases/01-evmwal-nft)
pnpm deploy:local     # forge deploy + sync address & ABI into the web app
...
- **Node 22** — `.nvmrc` at the repo root pins this (`nvm use` if you have nvm)
...
| | |
|---|---|
| Contracts | Foundry · Solidity ^0.8.24 · OpenZeppelin Contracts v5.1.0 (`ERC721` + `ERC721URIStorage` + `Ownable`) — packaged with the other showcase contracts in [`../contracts`](../contracts) |
...
| Package manager | pnpm 10 — this showcase is two workspace packages (`01-evmwal-nft` + `web`) |
...
```
showcases/01-evmwal-nft/
├── package.json                 deploy / extract-abi / seed / gen:assets scripts
├── assets/                      5 deterministic sample PNGs (legacy dev fixtures)
├── scripts/
...
The Solidity contract for this showcase lives in [`../contracts/src/EvmWalNFT.sol`](../contracts/src/EvmWalNFT.sol) together with the other showcase contracts; deploy script is [`../contracts/script/DeployEvmWalNFT.s.sol`](../contracts/script/DeployEvmWalNFT.s.sol).

## Scripts

Run these from `showcases/01-evmwal-nft/` unless noted.

| Command | What it does |
|---|---|
| `pnpm dev:chain` (from repo root) | Start Anvil on 127.0.0.1:8545 |
| `pnpm deploy:local` | Deploy `EvmWalNFT` to Anvil; write `.deployed-address`; sync ABI + address into `web/` |
| `pnpm extract-abi` | Re-sync `web/.env.local` + `web/lib/contract.ts` from the latest forge output |
...
| `pnpm test:upload-grep` | Source-grep AC tests for the backend upload feature |
| `pnpm test:upload-behavior` | Behavioral tests for the upload handler (stubbed SDK) |
| `pnpm format` (from repo root) | Prettier (+ `prettier-plugin-solidity` for `.sol`) |

## Tests

- `cd ../contracts && forge test --match-contract EvmWalNFT -vv` — 7 unit tests, 100% line coverage on `src/EvmWalNFT.sol`.
- `pnpm test:upload-grep` + `pnpm test:upload-behavior` — Node `node:test` suites covering the backend upload route.
